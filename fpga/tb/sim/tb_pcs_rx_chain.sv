...
module tb_pcs_rx_chain;
  ...
  task automatic drive_gearbox_sample(
  ...
  task automatic test_acquisition_and_slip_wait;
    begin
      ...
      if (block_lock !== 1'b0)
        $fatal(1, "Block lock asserted after reset");
      ...
      for (int i = 0; i < 20; i++) begin
        ...
        if (block_lock !== 1'b0)
          $fatal(1, "Block lock asserted during partial acquisition");
      end
      ...
      if (rx_gearbox_slip !== 1'b1)
        $fatal(1, "Expected gearbox-slip pulse after illegal header");

      if (block_lock !== 1'b0)
        $fatal(1, "Block lock asserted after illegal header");

      // The GT gearbox requires 32 RX clock cycles after a slip.
      for (int i = 0; i < 32; i++) begin
        drive_gearbox_sample(64'h0, 2'b00, 1'b0);

        if (rx_gearbox_slip !== 1'b0)
          $fatal(1, "Gearbox-slip pulse lasted longer than one cycle");

        if (block_lock !== 1'b0)
          $fatal(1, "Block lock asserted during slip wait");
      end
      ...
      for (int i = 0; i < 63; i++) begin
        ...
        if (block_lock !== 1'b0)
          $fatal(1, "Block lock asserted before 64 legal headers");
      end
      ...
      if (block_lock !== 1'b1)
        $fatal(1, "Block lock did not assert after 64 legal headers");
  ...
  task automatic test_acquisition_with_bubbles;
    begin
      ...
      for (int i = 0; i < 32; i++) begin
        ...
        if (block_lock !== 1'b0)
          $fatal(1, "Block lock asserted too early");
      end
      ...
      for (int i = 0; i < 7; i++) begin
        ...
        if (block_lock !== 1'b0)
          $fatal(1, "Block lock asserted during gearbox bubble");

        if (rx_gearbox_slip !== 1'b0)
          $fatal(1, "Gearbox bubble incorrectly caused a slip");
      end
      ...
      for (int i = 0; i < 31; i++) begin
        ...
        if (block_lock !== 1'b0)
          $fatal(1, "Block lock asserted before 64 real headers");
      end
      ...
      if (block_lock !== 1'b1)
        $fatal(1, "Gearbox bubbles incorrectly disturbed acquisition");
  ...
  task automatic acquire_lock;
    begin
      ...
      if (block_lock !== 1'b1)
        $fatal(1, "Failed to establish block lock");
    end
  endtask
  ...
  task automatic test_descrambler_stream;
    ...
    begin
      ...
      if (descrambled_payload_valid !== 1'b0)
        $fatal(1, "Descrambler exposed synchronization block");

      // Bubble must not advance the receiver LFSR.
      drive_gearbox_sample(64'h0, 2'b00, 1'b0);

      if (descrambled_payload_valid !== 1'b0)
        $fatal(1, "Descrambler asserted valid during gearbox bubble");
      ...
      for (int block_index = 0; block_index < 4; block_index++) begin
        ...
        if (descrambled_payload_valid !== 1'b1)
          $fatal(1, "Descrambler valid missing on block %0d", block_index);

        if (descrambled_payload !== plaintext)
          $fatal(1, "Descrambler mismatch on block %0d: expected %h, received %h", block_index, plaintext, descrambled_payload);

        if (header_out !== (block_index[0] ? 2'b01 : 2'b10))
          $fatal(1, "Header mismatch on block %0d", block_index);
      end
  ...
  task automatic test_lock_retention_and_window_rollover;
      begin
        ...
        for (int block_index = 0; block_index < 64; block_index++) begin
          ...
          if (block_lock !== 1'b1)
            $fatal(1, "Block lock lost below threshold at block %0d", block_index);

          if (rx_gearbox_slip !== 1'b0)
            $fatal(1, "Unexpected gearbox slip below lock-loss threshold");
        end
        ...
        if (block_lock !== 1'b1)
          $fatal(1, "Invalid-header count was not cleared at window rollover");

        if (rx_gearbox_slip !== 1'b0)
          $fatal(1, "Window rollover incorrectly caused a gearbox slip");
  ...
  task automatic test_lock_loss_and_reacquisition;
    begin
      ...
      for (int invalid_index = 0; invalid_index < 15; invalid_index++) begin
        ...
        if (block_lock !== 1'b1)
          $fatal(1, "Block lock lost before the 16th illegal header");

        if (rx_gearbox_slip !== 1'b0)
          $fatal(1, "Gearbox slip asserted before the 16th illegal header");
      end
      ...
      if (block_lock !== 1'b0)
        $fatal( 1, "Block lock did not clear on the 16th illegal header");

      if (rx_gearbox_slip !== 1'b1)
        $fatal(  1, "Gearbox slip was not asserted on lock loss" );
      ...
      for (int wait_cycle = 0; wait_cycle < 32; wait_cycle++) begin
        ...
        if (block_lock !== 1'b0)
          $fatal(1, "Block lock asserted during post-slip wait");

        if (rx_gearbox_slip !== 1'b0)
          $fatal(1, "Gearbox slip remained asserted for more than one cycle");
      end
      ...
      for (int header_index = 0; header_index < 63; header_index++) begin
        ...
        if (block_lock !== 1'b0)
          $fatal(1, "Block lock reacquired before 64 legal headers");
      end
    ...
    if (block_lock !== 1'b1)
      $fatal(1, "Block lock did not reacquire after 64 legal headers");
  ...
  task automatic test_reset_while_locked;
    ...
    begin
      ...
      if (descrambled_payload_valid !== 1'b1)
          $fatal(1, "Descrambler was not active before reset");

      if (descrambled_payload !== plaintext)
          $fatal(1, "Incorrect payload before reset");
      ...
      if (block_lock !== 1'b0)
        $fatal(1, "Block lock did not clear during reset");

      if (descrambled_payload_valid !== 1'b0)
        $fatal(1, "Descrambler valid remained asserted during reset");
      ...
      if (block_lock !== 1'b1)
        $fatal(1, "Block lock did not reacquire after reset");
      ...
      if (descrambled_payload_valid !== 1'b0)
        $fatal(1, "First block after reset was not suppressed");
      ...
      if (descrambled_payload_valid !== 1'b1)
        $fatal(1, "Descrambler did not resynchronize after reset");

      if (descrambled_payload !== plaintext)
        $fatal(1, "Post-reset payload mismatch: expected %h, received %h", plaintext, descrambled_payload);
