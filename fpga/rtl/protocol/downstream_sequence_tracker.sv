...
module downstream_sequence_tracker (
  ...
  assign current_message_has_sequence_mismatch = message_valid && !stream_fault && (message_sequence_number != expected_sequence_number_for_current_message);
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      else if (packet_commit && reset_all_recovery_pending) begin
        stream_fault <= 1'b0;
      end
      ...
      if (packet_discard) begin
        ...
        if (!stream_fault || reset_all_recovery_pending) begin
        ...
        else if (current_message_is_isolated_reset_all) begin
          current_packet_next_sequence_number <= next_sequence_number_after_current_message;
